...
public class FlowerFind {
	...
	public void flowerfind() {
		...
		if((((AliveFlower)(bou.getBouquet()[i])).getLength()>=min) && ((AliveFlower)(bou.getBouquet()[i])).getLength()<=max) {
			...
			if((bou.getBouquet()[i].getClass().getSimpleName().equals("NonLivingFlower") )){
				...
				if((((NonLivingFlower)(bou.getBouquet()[i])).getLength()>=min) && (((NonLivingFlower)(bou.getBouquet()[i])).getLength()<=max)) {
					System.out.println(bou.getBouquet()[i]);
				}		
			}
	}
